...
def check_syntax(file_path):
    try:
        ...
        construction_errors = validate_constructions(file_path)
        if construction_errors:
            return construction_errors
...
def validate_constructions(file_path):
    ...
    # Если ошибки есть, возвращаем их как часть программы
    if errors:
        return f"\n".join(errors)

    # Если ошибок нет, возвращаем None (что будет означать, что ошибок не было)
    return None
